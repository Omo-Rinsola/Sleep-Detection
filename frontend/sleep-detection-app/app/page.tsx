...
export default function SleepDetectionApp() {
  ...
  // Mock sleep detection status changes for demo
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    socketRef.current.onmessage = (event) => console.log(" From backend:", event.data)
...
// canvas snapshot
  useEffect(() => {
    ...
    // function that grabs snapshot
    const captureFrame = () => {
      ctx?.drawImage(video, 0, 0, canvas.width, canvas.height)
      const imageData = canvas.toDataURL("image/jpeg") // base64 string
      socketRef.current?.send(imageData) // send to backend
    }
